...
// SearchInfo (profiling stats)
py::class_<SearchInfo>(m, "SearchInfo")
.def_readonly("nodes",   &SearchInfo::nodes)
.def_readonly("tt_hits", &SearchInfo::ttHits)
.def_readonly("depth",   &SearchInfo::depthReached)
.def_readonly("time_ms", &SearchInfo::timeMs);
...
// optional getters if present:
.def("halfmove_clock",          &Board::getHalfmoveClock)
.def("fullmove_number",         &Board::getFullmoveNumber)
.def("en_passant_square",       &Board::getEnPassantSquare)
.def("castling_rights",         &Board::getCastlingRights)
.def("is_square_attacked",      &Board::isSquareAttacked, py::arg("sq"), py::arg("by"));
...
.def("last_search_info",  &Engine::lastSearchInfo);
}